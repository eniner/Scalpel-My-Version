...
class OverlayControllerGlobal {
    ...
    handler(e) {
        if (e.type === EventType.EVENT_ATTACH || e.type === EventType.EVENT_FOCUS) {
            try {
                require('fs').appendFileSync(require('path').join(process.env.APPDATA || '', 'scalpel', 'diagnostics', 'scalpel.log'), '[' + new Date().toISOString() + '] [overlay] native ' + e.type + ' w=' + (e.width || 0) + '\n');
            }
            catch (_h) { }
        }
        switch (e.type) {
    ...
    attachByTitle(electronWindow, targetWindowTitle, options = {}) {
        ...
        try {
            const handle = (_c = this.electronWindow) === null || _c === void 0 ? void 0 : _c.getNativeWindowHandle();
            const titles = Array.isArray(targetWindowTitle)
                ? targetWindowTitle
                : (targetWindowTitle === 'Path of Exile' || targetWindowTitle === 'Path of Exile 2')
                    ? ['Path of Exile', 'Path of Exile 2']
                    : [targetWindowTitle];
            const log = (msg) => {
                try {
                    require('fs').appendFileSync(require('path').join(process.env.APPDATA || '', 'scalpel', 'diagnostics', 'scalpel.log'), '[' + new Date().toISOString() + '] [overlay] ' + msg + '\n');
                }
                catch (_log) { }
            };
            try {
                lib.start(handle, titles, this.handler.bind(this));
                this.scalpelAttachMode = 'multi';
                log('lib.start ok titles=' + titles.join('|'));
            }
            catch (multiErr) {
                const one = (options && options.preferredTitle)
                    || (Array.isArray(targetWindowTitle) ? targetWindowTitle[0] : targetWindowTitle);
                lib.start(handle, one, this.handler.bind(this));
                this.scalpelAttachMode = 'single';
                log('lib.start ok title=' + one + ' (single-fallback) ' + multiErr);
            }
        }
        catch (e) {
            try {
                require('fs').appendFileSync(require('path').join(process.env.APPDATA || '', 'scalpel', 'diagnostics', 'scalpel.log'), '[' + new Date().toISOString() + '] [overlay] lib.start FAIL ' + e + '\n');
            }
            catch (_logFail) { }
            throw e;
        }
    ...
    attachByTitles(electronWindow, targetWindowTitles, options = {}) {
        this.attachByTitle(electronWindow, targetWindowTitles, options);
    }
    setTargetTitles(titles) {
        if (typeof lib.setTargetTitles !== 'function') {
            throw new Error('setTargetTitles not supported');
        }
