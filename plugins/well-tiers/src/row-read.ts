...
import { isRunicEntryName, pickBestRowDecode, RUNESHAPE_READER_VERSION } from './decode-row'
...
export async function readRewardRows(
  ...
  return {
    lines,
    candidates,
    method: `v${RUNESHAPE_READER_VERSION} ${pageKind} ${detectLabel}+${modeLabel} (${bands.length} rows)`,
  }
}
