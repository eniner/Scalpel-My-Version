  ...
  return (
    <div style={panelStyle} role="region" aria-label="Loot regression suite">
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        ...
        <button
          type="button"
          disabled={busy || running || suite.length === 0}
          style={{ fontSize: 10 }}
          ...
          return (
            <div key={row.id} style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: 10 }}>
              <span style={{ flex: 1, color: r?.changed ? '#fca5a5' : '#f0e6d2' }}>
                {row.label}
                {row.stackSize && row.stackSize > 1 ? ` ×${row.stackSize}` : ''} · expect{' '}
                {row.expectedVisibility ?? '?'} {row.expectedWinner ?? '—'}
...
export function EconomyPolicyPanel({
  ...
  const [preview, setPreview] = useState<
    Array<{ baseType: string; chaos: number; from: string; tier: FilterSectionTier }>
  >([])
  const [note, setNote] = useState<string | null>(null)
  ...
  const buildPreview = (): void => {
    ...
    setNote(
      `${rows.length} item(s) would affect ${new Set(rows.map((r) => r.tier.blockIndex)).size} tier(s) → ${action}`,
    )
  }
      ...
      {!section ? (
        ...
        <>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
            ...
            <select
              value={action}
              onChange={(e) => setAction(e.target.value as 'Hide' | 'Minimal' | 'Show')}
              style={inputStyle}
            >
    ...
    <div style={panelStyle} role="region" aria-label="Find by condition">
      ...
      <div style={{ maxHeight: 140, overflowY: 'auto', fontSize: 10 }}>
        {hits.length === 0 ? (
          ...
          hits.map((h) => (
            <div
              key={`${h.blockIndex}-${h.match}`}
              style={{ display: 'flex', gap: 6, alignItems: 'center', color: '#f0e6d2' }}
            >
              <span style={{ flex: 1 }}>
...
export function NamedCheckpointsPanel({ busy, onDone }: { busy?: boolean; onDone: () => void }): JSX.Element {
  ...
  const [versions, setVersions] = useState<
    Array<{ filename: string; label?: string; isCheckpoint: boolean; timestamp: number }>
  >([])
  const [note, setNote] = useState<string | null>(null)
  const [working, setWorking] = useState(false)
        ...
        {versions.map((v) => (
          <div key={v.filename} style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: 11 }}>
            ...
            <button
              ...
              style={{ fontSize: 10 }}
              onClick={() => void restore(v.filename, v.label)}
            >
              Restore
            </button>
          </div>
