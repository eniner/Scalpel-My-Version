...
export function FilterSectionEditor({ filterPath, variant = 'embedded' }: Props): JSX.Element {
  ...
  const moveMany = async (items: DragItem[], toBlockIndex: number): Promise<void> => {
    ...
    try {
      ...
      await refresh()
      pushToast(items.length === 1 ? `Moved ${items[0].baseType}` : `Moved ${items.length} items`, true)
    } finally {
      setBusy(false)
    }
  }
      ...
      for (const t of emptying) {
        ...
        try {
          ...
          if (block.ok && block.block) {
            const bases = block.block.conditions.filter((c) => c.type === 'BaseType').flatMap((c) => c.values)
  ...
  const copyStyleToTier = async (toBlockIndex: number): Promise<void> => {
    ...
    try {
      ...
      const result = await window.api.saveBlockEdit(
        toBlockIndex,
        { ...toRes.block, actions: [...kept, ...styleActions] },
        '',
      )
      if (!result.ok) {
  ...
  const duplicateTierNow = async (tier: FilterSectionTier): Promise<void> => {
    ...
    try {
      ...
      if (tier.baseTypes.length > 1) {
        ...
        const updated = secs.ok ? secs.sections.find((s) => s.typePath === active.typePath) : undefined
        const newTier = updated?.tiers.find((t) => t.tier === tierId && t.baseTypes.includes(tier.baseTypes[0]))
        if (newTier) {
  ...
  const onDragStart = (e: React.DragEvent, item: DragItem): void => {
    ...
    const moving = selected.some((s) => selKey(s) === key) && selected.length > 1 ? selected : [item]
    ...
    label.textContent = moving.length > 1 ? `${item.baseType} +${moving.length - 1}` : item.baseType
  ...
  const onDropItem = async (e: React.DragEvent, tier: FilterSectionTier, beforeIndex: number): Promise<void> => {
    e.preventDefault()
    e.stopPropagation()
    setDropTarget(null)
    setDropItemIndex(null)
    ...
    if (sameBlock && !hasCatalog) {
      const currentBases = isWindow && sortByPrice ? sortBasesByPrice(tier.baseTypes, prices) : tier.baseTypes
      ...
      if (addMode === 'existing') {
        ...
        if (names.length > 1) {
          ...
          const newTier = updated?.tiers.find(
            (t) => t.tier === (addNewTierId.trim() || 'custom') && t.baseTypes.includes(name),
          )
    ...
    >
      ...
      {showGuide ? (
        ...
        <>
                  ...
                  >
                    ...
                    {active && (
                      <button
                        ...
                        onClick={() =>
                          void moveBase(hit.baseType, hit.blockIndex, tiers[0]?.blockIndex ?? hit.blockIndex)
                        }
                        style={{ fontSize: 10, padding: '2px 8px' }}
                      >
                        Move here
          ...
          {canAdvanced && showLootSim && <SectionLootSim section={active} busy={busy} onBusy={setBusy} />}
          ...
          <div
            style={{ fontSize: 11, color: '#9a9aab', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}
          >
            {active ? (
              <>
          ...
          >
            {tiers.length === 0 ? (
              <div style={{ fontSize: 12, color: '#9a9aab', padding: '8px 2px' }}>
            ...
            {tiers.map((tier) => {
              ...
              const fileOrdered = active ? [...active.tiers].sort((a, b) => a.blockIndex - b.blockIndex) : []
              ...
              return (
                ...
                >
                  ...
                  {open && canEdit && <TierConditionInspector tier={tier} busy={busy} onSaved={() => void refresh()} />}
