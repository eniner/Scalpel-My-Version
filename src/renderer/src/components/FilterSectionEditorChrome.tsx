...
export function WorkflowStrip({
  ...
  return (
    ...
    >
      ...
      <button type="button" disabled={busy} onClick={onDiagnose} style={toolBtn(diagnoseOn)} title="Open What wins? + Filmstrip">
        1 · Diagnose
      </button>
      <span style={{ color: '#6b6b7a', fontSize: 12 }} aria-hidden>
        →
      </span>
      <button type="button" disabled={busy} onClick={onFix} style={toolBtn(fixOn)} title="Open Strictness + Economy policy">
        2 · Fix
      </button>
      <span style={{ color: '#6b6b7a', fontSize: 12 }} aria-hidden>
        →
      </span>
      <button type="button" disabled={busy} onClick={onVerify} style={toolBtn(verifyOn)} title="Open loot suite + Preflight">
        3 · Verify
      </button>
      <span style={{ fontSize: 10, color: '#6b6b7a', marginLeft: 4 }}>see problem → change filter → re-check drops</span>
    </div>
  )
}
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
        ...
        <button type="button" disabled={busy} aria-pressed={sortByPrice} onClick={onToggleSort} style={toolBtn(sortByPrice)}>
          Sort price
        </button>
