  ...
  const tiers = useMemo(() => {
    ...
    return section.tiers.filter(
      (t) =>
        ...
        t.tier.toLowerCase().includes(qq) ||
        t.baseTypes.some((b) => b.toLowerCase().includes(qq)),
    )
  }, [section, q])
  ...
  return (
    <div
      style={{
        ...
        padding: collapsed ? '6px 10px' : panelStyle.padding,
      }}
      role="region"
    ...
    >
      ...
      >
        ...
        <span>
          Possible duplicates ({conflicts.length}){collapsed ? ' — click to expand' : ' — same $type/$tier only'}
        </span>
      </button>
      {!collapsed && (
          ...
          {conflicts.slice(0, 30).map((c) => (
            <div key={c.baseType} style={{ marginBottom: 6 }}>
              ...
              <div style={{ color: '#9a9aab' }}>
                ...
                {c.hides.length > 0 ? ` · Hide: ${c.hides.map((s) => `${s.title}/${s.tierLabel}`).join(', ')}` : ''}
              </div>
            </div>
          ))}
        </div>
...
export function EconomyBar({
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', fontSize: 11, color: '#9a9aab' }}>
      <span>
        Economy:{' '}
        {loading ? 'loading…' : valued.length ? `${valued.length} priced · ~${sum.toFixed(0)}c section` : 'no prices'}
      </span>
      ...
      <button
        ...
        onClick={() => {
          void (async () => {
            ...
            const s = r.stats
            onMessage(
              s ? `Synced — +${s.added} / −${s.removed} / both ${s.bothChanged} / user ${s.userOnly}` : 'Synced',
            )
          })()
        }}
        style={{ fontSize: 11 }}
...
export async function duplicateTier(
  ...
  return {
    ok: true,
    error: tier.baseTypes.length > 1 ? `Duplicated with ${first}; re-add remaining after refresh` : undefined,
  }
}
...
export function useSectionPrices(
  section: FilterSection | null,
  league: string,
): {
  prices: Record<string, number | null>
