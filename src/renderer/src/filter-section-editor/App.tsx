...
export function FilterSectionEditorApp(): JSX.Element {
  ...
  const ensureFilterDir = useCallback(
    async (current: RuntimeSettings | null): Promise<RuntimeSettings | null> => {
  ...
  const sameGameProfiles = useMemo(
    () => profiles.filter((p) => p.gameVariant === poeVersion),
    [profiles, poeVersion],
  )
      ...
      <div className="flex-1 min-h-0 overflow-hidden p-3 flex flex-col gap-2">
        ...
        {!filterDir && (
          <p className="text-[12px] text-text-dim m-0">
            ...
            <strong>Browse</strong> to your Path of Exile filters folder — then pick e.g. “3.29 RF Campaign…”
