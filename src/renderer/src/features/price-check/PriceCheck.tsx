...
export function PriceCheck({
  ...
  const [rawBulkListings, setRawBulkListings] = useState<BulkListing[]>([])
  const [excludeOwn, setExcludeOwn] = useState(false)
  const accountName = auth?.loggedIn ? auth.accountName : undefined
  const listings = useMemo(
    () => filterOwnListings(rawListings, accountName, excludeOwn),
    [rawListings, accountName, excludeOwn],
  )
  const bulkListings = useMemo(
    () => filterOwnListings(rawBulkListings, accountName, excludeOwn),
    [rawBulkListings, accountName, excludeOwn],
  )
  ...
  useEffect(() => {
    ...
    window.api.getSettings().then((s) => {
      ...
      if (s.tradeExcludeOwnListings) setExcludeOwn(true)
  ...
  const doBulkSearch = async (): Promise<void> => {
    ...
    try {
      ...
      setRawBulkListings(result.listings)
  ...
  const loadMore = async (): Promise<void> => {
    ...
    try {
      ...
      setRawListings((prev) => [...prev, ...result.listings])
      setRemainingIds(result.remainingIds)
    } catch {
      // silently fail
    } finally {
    ...
    <div className="flex flex-col flex-1 min-h-0 bg-bg">
      ...
      <div className="flex-1 overflow-y-auto px-[14px] py-[10px] flex flex-col gap-[10px]">
        ...
        {!isBulk &&
          (filters.some((f) => f.type === 'socket' || f.type === 'misc') ||
            ...
            <div className="flex gap-[6px] flex-wrap">
              ...
              {/* Hide the logged-in account's own listings from trade results. */}
              <span title={!loggedIn ? m.price_check_exclude_own_login() : undefined}>
                <FilterChip
                  label={m.price_check_exclude_own()}
                  active={excludeOwn}
                  disabled={!loggedIn}
                  onClick={() => {
                    if (!loggedIn) return
                    const next = !excludeOwn
                    setExcludeOwn(next)
                    void window.api.setSetting('tradeExcludeOwnListings', next)
                  }}
                />
              </span>
        ...
        {isBulk && searched && !searching && bulkListings.length === 0 && !error && (
          <div className="text-[11px] text-text-dim text-center p-2">
            {excludeOwn && rawBulkListings.length > 0 ? m.price_check_no_other_sellers() : 'No sellers found'}
          </div>
        )}
        ...
        {!isBulk && searched && !searching && listings.length === 0 && !error && (
          <div className="text-[11px] text-text-dim text-center p-2">
            {excludeOwn && rawListings.length > 0 ? m.price_check_no_other_sellers() : 'No listings found'}
          </div>
        )}
      </div>
