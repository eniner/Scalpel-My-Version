...
import type { ActiveFilter, FilterType, UniqueTierGrade } from './types'
import { ALL_FILTER_TYPES, FILTER_LABELS, TIER_COLORS, TIER_ORDER } from './constants'
import { formatDust, scaleRange } from './utils'

interface FilterRowProps {
  filter: ActiveFilter
  idx: number
  availableTypes: FilterType[]
  minValues: Record<'chaosValue' | 'dustIlvl84', number>
  maxValues: Record<'chaosValue' | 'dustIlvl84', number>
  divineRate: number
  mirrorRate: number
  ...
  return (
    <div className="flex items-center gap-[6px]">
      <select
        ...
        className="shrink-0 text-[11px] h-[25px] leading-[25px] w-[110px] box-border cursor-pointer rounded"
      ...
      {f.type === 'name' ? (
        ...
        <div className="flex-1 flex items-center gap-1 h-[25px]">
          {TIER_ORDER.map((tier) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => {
                  const next = on ? f.tiers.filter((t) => t !== tier) : [...f.tiers, tier]
                  onUpdate(idx, { tiers: next })
                }}
                className="h-[22px] min-w-[26px] px-1.5 rounded text-[11px] font-bold"
                style={{
                  color: on ? TIER_COLORS[tier] : 'var(--text-dim)',
                  background: on ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.25)',
                  border: on ? `1px solid ${TIER_COLORS[tier]}55` : '1px solid transparent',
                }}
              >
                {tier}
              </button>
        ...
        <div className="flex-1 flex items-center gap-1 bg-black/30 rounded-full px-[6px] relative overflow-hidden h-[25px] box-border">
          ...
          <span className="text-[10px] font-mono text-accent font-semibold shrink-0 min-w-[26px] text-right">
            {formatFilterVal(
              f.type,
              scaleRange(f.min, minValues[f.type], maxValues[f.type], f.type),
              divineRate,
              mirrorRate,
            )}
          </span>
          ...
          <span className="text-[10px] font-mono text-accent font-semibold shrink-0 min-w-[26px] text-left">
            {formatFilterVal(
              f.type,
              scaleRange(f.max, minValues[f.type], maxValues[f.type], f.type),
              divineRate,
              mirrorRate,
            )}
          </span>
  ...
  return (
    <div className="flex items-center gap-[6px]">
      <select
        ...
        className="shrink-0 text-[11px] text-text h-[25px] leading-[25px] w-[110px] box-border cursor-pointer rounded"
        style={{ padding: '0 24px 0 8px' }}
      >
...
export function defaultFilter(type: FilterType): ActiveFilter {
  return {
    type,
    value: '',
    min: 0,
    max: 1000,
    tiers: [...TIER_ORDER] as UniqueTierGrade[],
  }
}
