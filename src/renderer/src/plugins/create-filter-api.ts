...
export function createFilterApi(api: HostApi = window.api): FilterApi {
  return {
    ...
    saveBlockEdit: (blockIndex, block, itemJson) =>
      api.saveBlockEdit(blockIndex, block as Parameters<HostApi['saveBlockEdit']>[1], itemJson),
    ...
    insertSectionRule: (opts) => api.insertSectionRule(opts as Parameters<HostApi['insertSectionRule']>[0]),
    applySectionDelta: (req) => api.applySectionDelta(req as Parameters<HostApi['applySectionDelta']>[0]),
    matchItem: (req) => api.matchFilterItem(req as Parameters<HostApi['matchFilterItem']>[0]),
    ...
    simulateLootDrops: (req) => api.simulateLootDrops(req as Parameters<HostApi['simulateLootDrops']>[0]),
