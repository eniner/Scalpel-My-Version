...
type InstallUpdateResult = { ok: true } | { ok: false; error: string }

function removeWritableFile(path: string): void {
  if (!existsSync(path)) return
  if (process.platform === 'win32') {
    try {
      execSync(`attrib -R "${path.replace(/"/g, '""')}"`, { windowsHide: true })
    } catch {
      /* ignore */
    }
  }
...
function removeLegacyApplyScripts(userDataDir: string): void {
  for (const name of ['apply-update.vbs', 'apply-update.bat']) {
    try {
      removeWritableFile(join(userDataDir, name))
    } catch {
      /* ignore */
    }
  }
...
function writeUpdateLauncherBat(lines: string[]): string {
  const batPath = join(tmpdir(), `scalpel-apply-update-${process.pid}.bat`)
  removeWritableFile(batPath)
  writeFileSync(batPath, lines.join('\r\n'), { encoding: 'utf8' })
  return batPath
}

function launchDetachedBatch(batPath: string): void {
...
ipcMain.handle('install-update', (): InstallUpdateResult => {
  ...
  try {
    removeLegacyApplyScripts(userDataDir)
    ...
    const batPath = writeUpdateLauncherBat(batLines)
    launchDetachedBatch(batPath)
    ...
    recordMainBreadcrumb('updater: install-update failed', { error: message })
