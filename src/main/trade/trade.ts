...
function statFilterValue(f: StatFilter): Record<string, unknown> {
  if (f.option) return { option: f.option }
  const mm = minMaxValue(f)
  return Object.keys(mm).length > 0 ? mm : {}
}
...
export async function searchTrade(
  ...
  const { tradeStatus = 'available', tradePriceOption, listedTime, collapseListings = true, loggedIn = true, statMatchMode = 'and' } = options
