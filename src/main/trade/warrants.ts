...
function mercenaryName(properties: Array<{ name?: string; values?: Array<[string, number]> }> | undefined): string {
...
export function groupListings(listings: WarrantListing[], rank: 'floor' | 'median' = 'floor'): WarrantSkillGroup[] {
...
export async function scanMercenaryWarrants(
  ...
  const maxAskDivine = options.maxAskDivine === undefined ? DEFAULT_MAX_ASK_DIVINE : options.maxAskDivine
  ...
  if (wantSkills.length > 0 && skillStats.resolved === 0) {
    throw new Error(`No trade filters found for: ${wantSkills.join(', ')}. Try the exact skill name from a warrant.`)
  }
  ...
  const allListings: WarrantListing[] = raw.map((r) => {
    ...
    const chaosValue = typeof amount === 'number' && currency ? priceToChaos(amount, currency) : null
