...
describe('searchTrade filter-group dispatch', () => {
  ...
  it('statMatchMode any emits a count group for explicit stats', async () => {
    setPoeVersion(2)
    const shield = {
      name: '',
      baseType: '',
    ...
    await searchTrade('Runes of Aldur', shield, statFilters, {
      tradeStatus: 'any',
      tradePriceOption: 'exalted_divine',
      statMatchMode: 'any',
    })
    ...
    expect(countGroup.filters).toHaveLength(2)
    expect(body.query.stats.some((g: { type: string }) => g.type === 'and')).toBe(false)
  })
