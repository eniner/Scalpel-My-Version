...
describe('ITEM_CLASS_TO_CATEGORY', () => {
  it('maps common item classes to trade categories', () => {
    ...
    expect(ITEM_CLASS_TO_CATEGORY.Charms).toBe('accessory.charm')
...
describe('buildEnchantFilters corruption implicit fallback (The Vertex)', () => {
  afterEach(() => {
    _setStatEntriesForTests([])
  })

  it('maps a { Corruption Enhancement } curse-level line to the implicit trade stat', () => {
    _setStatEntriesForTests([
      { id: 'implicit.stat_curse123', text: '+# to Level of all Curse Skills', type: 'implicit' },
    ])
    const filters = matchItemMods(
      ...
      undefined,
      {
        ...
        enchants: ['+5 to Level of all Curse Skills'],
      },
    )
    const row = filters.find((f) => f.text.includes('Curse Skills'))
    expect(row).toBeDefined()
    expect(row?.id).toBe('implicit.stat_curse123')
    expect(row?.type).toBe('implicit')
    expect(row?.enabled).toBe(true)
    expect(row?.min).toBe(5)
    expect(row?.max).toBe(5)
  })

  it('still prefers enchant.* when an enchant stat exists', () => {
    _setStatEntriesForTests([
      { id: 'enchant.stat_spirit', text: '#% increased Spirit', type: 'enchant' },
      { id: 'implicit.stat_spirit', text: '#% increased Spirit', type: 'implicit' },
    ])
    const filters = matchItemMods([], [], undefined, {
      ...makeItemInfo({ itemClass: 'Sceptres', baseType: 'Omen Sceptre', rarity: 'Unique', corrupted: true }),
      enchants: ['24% increased Spirit'],
    })
    const row = filters.find((f) => f.text.includes('Spirit'))
    expect(row?.id).toBe('enchant.stat_spirit')
    expect(row?.type).toBe('enchant')
  })
})
